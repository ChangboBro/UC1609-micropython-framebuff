...
class UC1609(framebuf.FrameBuffer):
    def __init__(self,spi,dc,cs,rst,potMeter=0x49,biasRatio=0x03,invX=0,invY=0,invdisp=0):
    ...
    def initscreen(self):
        ...
        self.fill(0)
        self.show()
        self.write_cmd(DISP_ON|0x01)#1:display on normal display mode
    ...
    def show(self):
        #write_cmd(ADDR_CTRL)
        write_cmd(DISP_ON|0x00)
        write_cmd(SET_CAL)
        write_cmd(SET_CAH)
        write_cmd(SET_PA)
        write_data(self.buffer)
        write_cmd(DISP_ON|0x01)